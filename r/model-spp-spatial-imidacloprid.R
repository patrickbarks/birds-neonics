...
opts <- docopt('
  Usage:
  spp-model-spatial-imidacloprid.R [-y <year> -c <cores> -s <species>]
...
for (i in indices) {
  ...
  fips_by_bcr <- bbs_filter %>%
    ...
    left_join(imidacloprid_counties, by = 'fips') %>% 
    group_by(bcr) %>% 
    mutate(imid_mean_cent = imid_scale - mean(imid_scale)) %>% ungroup() %>% 
  ...
  stan_dat <- list(
    ...
    neonic = fips_by_bcr$imid_mean_cent
  )
  ...
  file_out <- paste0('stanfit/spatial-imidacloprid/fit-imid-county-nested4-', aou_focal, '-', min(years_focal), 'to', max(years_focal), '.RData')
  ...
  if(n_diverg > 30 | rhat_high > 0) {
    ...
    file_out_redo <- paste0('stanfit/spatial-imidacloprid/fit-imid-county-nested4-redo-', aou_focal, '-', min(years_focal), 'to', max(years_focal), '.RData')
