...
opts <- docopt('
  Usage:
  stan-spatial-neonic.R [-y <year> -c <cores> -s <species>]
...
for (i in indices) {
  ...
  file_out <- paste0('stanfit/spatial-neonic/fit-neonic-county-nested4-', aou_focal, '-', min(years_focal), 'to', max(years_focal), '.RData')
  ...
  if(n_diverg > 30 | rhat_high > 0) {
    ...
    file_out_redo <- paste0('stanfit/spatial-neonic/fit-neonic-county-nested4-redo-', aou_focal, '-', min(years_focal), 'to', max(years_focal), '.RData')
